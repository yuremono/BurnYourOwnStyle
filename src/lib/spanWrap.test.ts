...
describe("initSpanWrap", () => {
	it("テキストノード内の < をエスケープし、新たな要素として解釈されない", () => {
		...
		const el = document.querySelector(".js-letter > span")!
	...
	it("通常の文字は span で分割される", () => {
		document.body.innerHTML = '<div class="js-letter"><span>ab</span></div>'
		initSpanWrap(document.body)
		const inner = document.querySelector(".js-letter > span")!.innerHTML
