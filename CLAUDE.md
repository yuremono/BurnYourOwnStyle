...
完成後はこのプロジェクトを丸ごとコピーして新規プロジェクトの基盤としての使用を想定する。
...
- 変数名はキャメルケース （現状は1単語または大文字の略語）例： var(--wid), var(--PX)
**コンポーネント内で新たなクラス名をつける場合もパスカルケースを使用**
...
- **勝手に名前をつける**  クラス名、スキル名、コンポーネント名、Unit名など、エージェントがそれらしい名前をつけることはしない。
  名前が必要な場合はユーザーに提案し、承認を得ること。

- **Tailwind以外でケバブケースを使う**
  プロジェクト固有はパスカルケース。Tailwindとの差別化という目的もある。
