...
プロジェクト固有の`CustomClass `を元にwebページ,アプリを作成する
...
- ユーザーの口調が、まだ調査・検討・熟考の段階にあることを示しているかどうかを再考してください。  
- ClassName と ComponentName を独断で決定しないでください — 必ず提案して承認を得てください  
- いかなる識別子にもプロジェクト名を使用しないでください  
- パス内のユーザー名は `{PROJECT_ROOT}` のようなプレースホルダに置き換えてください  
- 秘密情報をハードコードしないでください — 代わりに環境変数または設定ファイルを使用してください
</important>

<important if="overwriting, deleting, or resetting">
- ユーザーの口調が、まだ調査・検討・熟考の段階にあることを示しているかどうかを再考してください。  
- 承認なしにコメントを削除しないでください  
- 仮定に基づいて進めないでください
</important>
