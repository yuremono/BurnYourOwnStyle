...
**Cursor 用は `.cursor-memsearch/` を使用。`.memsearch/`（Claude Code 用）とは完全分離。**

```
PROJECT_ROOT/
├── .cursor-memsearch/           # Cursor 専用（.memsearch とは別）
│   ├── .memsearch.toml          # Cursor 用 config（collection 分離）
│   ├── memory/
│   │   ├── 2026-03-18.md
│   │   └── ...
│   └── .watch.pid
├── .cursor/
│   ├── hooks.json               # 4 フック登録済み
│   ├── hooks/
│   │   ├── session-start.sh
│   │   ├── before-submit-prompt.sh
│   │   ├── stop.sh
│   │   ├── session-end.sh
│   │   ├── common-cursor-memsearch.sh
│   │   └── lib/
│   │       ├── parse-cursor-transcript.py
│   │       └── parse-cursor-transcript.sh
│   └── skills/
│       └── memory-recall/
│           └── SKILL.md         # Cursor 用メモリ検索スキル
└── .memsearch/                  # Claude Code 用（既存・変更なし）
```
...
**前提条件:** `memsearch` CLI が PATH にあり、`.cursor-memsearch/memory/` がインデックス済みであること。

**プロジェクト内スキル:** `.cursor/skills/memory-recall/SKILL.md` に Cursor 用の memory-recall スキルを配置済み。検索時は `cd .cursor-memsearch && memsearch search "..."` で Cursor 用インデックスを参照する。
...
| **watch プロセス** | sessionStart で `memsearch watch .cursor-memsearch/memory/` を起動し、sessionEnd で停止 |
| **要約モデル** | stop.sh で `claude -p --model haiku` を使用。未導入時は先頭 800 文字をフォールバックとして追記 |
...
- Phase 0〜5 完了。フック・パーサー・スキルを配置済み。
- 初回 Cursor チャット利用時、stop フックで会話が `.cursor-memsearch/memory/YYYY-MM-DD.md` に追記される。
- sessionStart で直近メモリがコンテキストに注入される。

### トラブルシュート

| 現象 | 対処 |
|------|------|
| transcript_path が null | Cursor 設定でトランスクリプト保存が有効か確認。無効の場合は stop フックはスキップされる |
| memsearch が見つからない | `pip install "memsearch[onnx]"` または `uvx --from 'memsearch[onnx]' memsearch` で実行 |
| 要約がフォールバックになる | `claude` CLI が PATH にない場合、先頭 800 文字がそのまま追記される。Claude Code 利用環境なら `claude` が使える可能性あり |
| 検索結果が空 | `.cursor-memsearch/memory/` に .md が存在し、`memsearch index ./memory/` が実行済みか確認 |

### 手動インデックス更新

```bash
cd .cursor-memsearch && memsearch index ./memory/ --force
```
...
- 詳細実装計画: [MEMSEARCH_CURSOR_IMPLEMENTATION_PLAN.md](MEMSEARCH_CURSOR_IMPLEMENTATION_PLAN.md)
