...
async function testOverride() {
  ...
  // Tailwind の gap-4 = 1rem = 16px
    // Burn Your Own Style のデフォルト gap = 30px
    // Burn Your Own Style が Tailwind よりも後に来るので、30px になるはず
  ...
  if (cardsGap.computedGap === '30px') {
      console.log('   ✅ Burn Your Own Style gap が適用されている（Tailwind より優先）');
  } else {
    console.log('   ❌ Tailwind gap が優先されている');
  }

  // Test: Check specific item gap
    console.log('\n2. Checking .item gap (should be 1em = Burn Your Own Style)...');
  ...
  if (itemGap === '1em' || itemGap === '16px') {
      console.log('   ✅ Burn Your Own Style .item gap が適用されている');
  }

    // Test: Check if Tailwind classes override Burn Your Own Style CSS
  ...
  const tailwindOverrides = await page.evaluate(() => {
    ...
    return {
      ...
      borderRadius: getComputedStyle(card1).borderRadius,
        // Check if Burn Your Own Style .Cards gap (30px) is on parent
  ...
  console.log(`   - borderRadius (should be 0.5rem / 8px): ${tailwindOverrides.borderRadius}`);
    console.log(`   - parent Gap (Burn Your Own Style .Cards, should be 30px): ${tailwindOverrides.parentGap}`);

  await browser.close();

  console.log('\n=== Summary ===');
    console.log('Burn Your Own Style (.Cards, .item) は Tailwind より後に定義。');
